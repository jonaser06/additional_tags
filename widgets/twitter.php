...
class themerex_widget_twitter extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_twitter', 'description' => __('Last Twitter Updates. Version for new Twitter API 1.1', 'additional-tags') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'themerex_widget_twitter' );

		/* Create the widget. */
		parent::__construct( 'themerex_widget_twitter', __('ThemeREX - Twitter', 'additional-tags'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		foreach ($data as $tweet) {
			...
			$output .= '<li class="theme_text' . ($cnt==$twitter_count-1 ? ' last' : '') . '"><a href="' . esc_url('https://twitter.com/'.($twitter_username)) . '" class="username" target="_blank">@' . ($tweet['user']['screen_name']) . '</a> ' . themerex_prepare_twitter_text($tweet) . '</li>';
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'description' => __('Last Twitter Updates', 'additional-tags') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_count' )); ?>"><?php _e('Tweets count:', 'additional-tags'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_username' )); ?>"><?php _e('Twitter Username:', 'additional-tags'); ?><br />(<?php _e('leave empty if you paste widget code', 'additional-tags'); ?>)</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_consumer_key' )); ?>"><?php _e('Twitter Consumer Key:', 'additional-tags'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_consumer_secret' )); ?>"><?php _e('Twitter Consumer Secret:', 'additional-tags'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_token_key' )); ?>"><?php _e('Twitter Token Key:', 'additional-tags'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_token_secret' )); ?>"><?php _e('Twitter Token Secret:', 'additional-tags'); ?></label>
