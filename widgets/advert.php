...
class themerex_widget_advert extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_advert', 'description' => __('Advertisement block', 'additional-tags') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'themerex_widget_advert' );

		/* Create the widget. */
		parent::__construct( 'themerex_widget_advert', __('ThemeREX - Advertisement block', 'additional-tags'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_advert_inner">
			...
			if ($advert_code!='') {
				themerex_show_layout(themerex_substitute_all($advert_code));
			}
			?>
		</div>
		<?php
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'description' => __('Advertisement block', 'additional-tags') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_image' )); ?>"><?php _e('Image source URL:<br />(leave empty if you paste advert code)', 'additional-tags'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_link' )); ?>"><?php _e('Image link URL:<br />(leave empty if you paste advert code)', 'additional-tags'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_code' )); ?>"><?php _e('or paste Advert Widget HTML Code:', 'additional-tags'); ?></label>
