...
if ( !function_exists( 'trx_addons_enqueue_wp_color_picker' ) ) {
	function trx_addons_enqueue_wp_color_picker() {
		...
		wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
			'clear' => __( 'Clear', 'additional-tags'),
			'defaultString' => __( 'Default', 'additional-tags'),
			'pick' => __( 'Select Color', 'additional-tags'),
			'current' => __( 'Current Color', 'additional-tags'),
		)); 
...
if ( !function_exists( 'trx_addons_enqueue_googlemap' ) ) {
	function trx_addons_enqueue_googlemap() {
		...
		if (trx_addons_is_on(themerex_get_theme_option('api_google_load')) & !empty($api_key)) {
			wp_enqueue_script( 'google-maps', trx_addons_get_protocol().'://maps.googleapis.com/maps/api/js?key='.$api_key, array(), null, true );
		}
	}
}
		...
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				...
				if (trx_addons_is_on(themerex_get_theme_option('debug_mode', false, false)))
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Put contents to the file "%s" failed', 'additional-tags'), $file));
			}
		}
		return false;
	}
	...
	function trx_addons_fgc($file, $unpack=false) {
		...
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				...
				if (trx_addons_is_on(themerex_get_theme_option('debug_mode', false, false)))
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get contents from the file "%s" failed', 'additional-tags'), $file));
			}
		}
		return '';
	}
	...
	function trx_addons_fga($file) {
		...
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				...
				if (trx_addons_is_on(themerex_get_theme_option('debug_mode', false, false)))
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get rows from the file "%s" failed', 'additional-tags'), $file));
			}
		}
		return array();
	}
