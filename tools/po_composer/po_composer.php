...
class themerex_po_composer {
	...
	function admin_menu_item() {
		...
		add_management_page(__('PO Composer', 'additional-tags'), __('PO Composer', 'additional-tags'), 'manage_options', 'po_composer', array($this, 'build_page'));
	}
	...
	function build_page() {
		...
		$options  =  "\n".'<option value="upload_">'.__('Upload .po-file', 'additional-tags').'</option>'
					."\n".'<option value="edit_"'.(in_array($this->po_src, array('upload_', 'edit_')) ? ' selected="selected"' : '').'>'.__('Edit .po-text', 'additional-tags').'</option>'
					."\n".'<optgroup label="'.($this->is_child ? __('Parent languages', 'additional-tags') : __('Languages', 'additional-tags')).'">'.trim($this->get_list_options($parent_list,  'parent_', $this->po_src))."\n".'</optgroup>'
					.($this->is_child ? "\n".'<optgroup label="'.__('Child languages', 'additional-tags').'">'.trim($this->get_list_options($child_list, 'child_', $this->po_src))."\n".'</optgroup>' : '');
		$options2 = "\n".'<option value="">'.__('- Select merge source -', 'additional-tags').'</option>'
					."\n".'<option value="upload_"'.($this->error && $this->po_src2=='upload_' ? ' selected="selected"' : '').'>'.__('Upload .po-file', 'additional-tags').'</option>'
					."\n".'<option value="edit_"'.($this->error && $this->po_src2=='edit_' ? ' selected="selected"' : '').'>'.__('Edit .po-text', 'additional-tags').'</option>'
					."\n".'<optgroup label="'.($this->is_child ? __('Parent languages', 'additional-tags') : __('Languages', 'additional-tags')).'">'.trim($this->get_list_options($parent_list,  'parent_', $this->error ? $this->po_src2 : ''))."\n".'</optgroup>'
					.($this->is_child ? "\n".'<optgroup label="'.__('Child languages', 'additional-tags').'">'.trim($this->get_list_options($child_list, 'child_', $this->error ? $this->po_src2 : ''))."\n".'</optgroup>' : '');

		?>
		<div class="po_composer">
			<h2 class="po_composer_title"><?php _e('ThemeREX .PO-files Composer', 'additional-tags'); ?></h2>
			...
			<form id="po_composer_form" action="#" method="post" enctype="multipart/form-data">
				...
				<div class="po_composer_block">
					<fieldset class="po_composer_block_inner">
						<legend><?php _e('Edit .po-file:', 'additional-tags'); ?></legend>
						<div class="po_composer_fields">
							<div class="po_composer_field">
								<label for="po_src"><?php _e('Select .po-file:', 'additional-tags'); ?></label>
							...
							<div class="po_composer_field po_composer_editor">
								...
								<input type="checkbox" value="1" name="po_translated_down" id="po_translated_down"<?php echo isset($_POST['po_translated_down']) ? ' checked="checked"' : ''; ?> /><label for="po_translated_down"><?php _e('Move translated strings down', 'additional-tags'); ?></label>
								</span>
								<ul class="right_side">
									<li id="po_composer_link_text" class="po_composer_link_text"><a href="#po_composer_editor_text"><?php _e('Plain text', 'additional-tags'); ?></a></li><li id="po_composer_link_list" class="po_composer_link_list"><a href="#po_composer_editor_list"><?php _e('Strings editor', 'additional-tags'); ?></a></li>
								</ul>
				...
				<div class="po_composer_block">
					<fieldset class="po_composer_block_inner">
						<legend><?php _e('Merge with .po-file:', 'additional-tags'); ?></legend>
						<div class="po_composer_fields">
							<div class="po_composer_field">
								<label for="po_src2"><?php _e('Select .po-file:', 'additional-tags'); ?></label>
				...
				<div class="po_composer_buttons">
					<a href="#" id="po_save"><?php echo ($this->po_src=='' ? __('Upload', 'additional-tags') : __('Update', 'additional-tags')); ?></a>
					<?php if (!empty($this->po_link) || !empty($this->mo_link)) { ?>
					<div class="po_composer_links">
						<?php if (!empty($this->po_link)) { ?>
							<a href="<?php echo esc_url($this->po_link); ?>" class="po_composer_download_link"><?php _e('Download .PO-file', 'additional-tags'); ?></a>
						<?php } ?>
						<?php if (!empty($this->mo_link)) { ?>
							<a href="<?php echo esc_url($this->mo_link); ?>" class="po_composer_download_link"><?php _e('Download .MO-file', 'additional-tags'); ?></a>
						<?php } ?>
					</div>
					<?php } ?>
				</div>
	...
	function actions_handler() {
		if (!empty($this->po_src)) {
			if ( !isset($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], __FILE__ ) ) {
				$this->error = __('Incorrect WP-nonce data! Operation canceled!', 'additional-tags');
			} else {
				do {
					...
					if (($po_data = $this->parse_po($po_text))===false) {
						$this->error = __('Error parsing new .po-file!', 'additional-tags');
						break;
					}
					if ($this->po_src2 != '') {
						...
						if (($po_data2 = $this->parse_po($po_text2))===false) {
							$this->error = __('Error parsing merging .po-file!', 'additional-tags');
							break;
						}
					}
			...
			if (empty($this->error)) {
				...
				if (empty($this->po_src2) && !empty($_POST['po_text']) && substr($this->po_src, 0, 7)=='parent_' || substr($this->po_src, 0, 6)=='child_') {
					...
					if (is_writeable($po_file)) {
						...
						$this->success = sprintf(__('Language file "<b>%s.po</b>" was updated!', 'additional-tags'), $name);
						$mo_file = trim($dir).'/'.trim($name).'.mo';
						if (is_writeable($mo_file)) {
							themerex_fpc($mo_file, $this->mo_text);
							$this->success .= '<br>'.sprintf(__('Language file "<b>%s.mo</b>" was generated!', 'additional-tags'), $name);
						}
					} else {
						$tjis->error_update = sprintf(__('Error writing in language file "<b>%s.po</b>"!<br>Please, copy text from textarea below and paste it in the .po-file manually<br>or use links to upload .po and .mo files below the text area!', 'additional-tags'), $name);
					}
				}
			} else {
				$this->po_text = stripslashes($_POST['po_text']);
	...
	function load_po($suffix='') {
		...
		do {
			...
			if ($_POST['po_src'.trim($suffix)] == 'upload_') {
				...
				if (empty($rez['data'])) {
					$rez['error'] = sprintf(__('Error uploading or Empty .po-file: %s', 'additional-tags'), $_FILES['po_file'.($suffix)]['tmp_name']);
					break;
				}
			// or get content from textarea
				...
				if (!empty($_POST['po_text'.($suffix)]))
					...
					$rez['error'] = __('Empty textarea with .po-file content!', 'additional-tags');
					break;
				}
			// or load file from 'languages' folder
				...
				if (empty($rez['data'])) {
					$rez['error'] = sprintf(__('Error loading or Empty .po-file: %s', 'additional-tags'), ($dir).'/'.($name).'.po');
					break;
				}
	...
	function get_list_files($dir, $ext) {
		$list = themerex_get_list_files($dir, $ext);
		for ($i=0; $i<count($list); $i++) {
			$list[$i] = basename($list[$i]);
			$list[$i] = substr($list[$i], 0, strrpos($list[$i], '.'));
		}
		return $list;
	}
