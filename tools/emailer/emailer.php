...
class themerex_emailer {
	...
	function admin_menu_item() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			//add_theme_page(__('Emailer', 'additional-tags'), __('Emailer', 'additional-tags'), 'edit_theme_options', 'trx_emailer', array($this, 'build_page'));

			// In this case menu item is add in admin menu 'Tools'
			add_management_page(__('Emailer', 'additional-tags'), __('Emailer', 'additional-tags'), 'manage_options', 'trx_emailer', array($this, 'build_page'));
		}
	}
	...
	function build_page() {
		...
		if ( isset($_POST['emailer_subject']) ) {
			do {
				...
				if (empty($subject)) {
					$this->error = __('Subject can not be empty! Operation canceled!', 'additional-tags');
					break;
				}
				$message = themerex_get_value_gp('emailer_message');
				if (empty($message)) {
					$this->error = __('Message can not be empty! Operation canceled!', 'additional-tags');
					break;
				}
				...
				if (count($subscribers)==0) {
					$this->error = __('Subscribers lists are empty! Operation canceled!', 'additional-tags');
					break;
				}
				...
				if (empty($sender_email)) {
					$this->error = __('Sender email is empty! Operation canceled!', 'additional-tags');
					break;
				}
				...
				if ($subscribers_update && $group!='none') {
					...
					$add_msg = __(' The subscriber list is updated', 'additional-tags');
				} else if ($subscribers_delete && $group!='none') {
					unset($this->subscribers[$group]);
					update_option('themerex_emailer_subscribers', $this->subscribers);
					$add_msg = __(' The subscriber list is cleared', 'additional-tags');
				}
				$this->success = __('E-Mail was send successfull!', 'additional-tags') . $add_msg;
			} while (false);
		}

		?>
		<div class="trx_emailer">
			<h2 class="trx_emailer_title"><?php _e('ThemeREX Emailer', 'additional-tags'); ?></h2>
			...
			<form id="trx_emailer_form" action="#" method="post" enctype="multipart/form-data">
				...
				<div class="trx_emailer_block">
					<fieldset class="trx_emailer_block_inner">
						<legend> <?php _e('Letter data', 'additional-tags'); ?> </legend>
						<div class="trx_emailer_fields">
							<div class="trx_emailer_field trx_emailer_subject">
								<label for="emailer_subject"><?php _e('Subject:', 'additional-tags'); ?></label>
							...
							<div class="trx_emailer_field trx_emailer_attachment">
								<label for="emailer_attachment"><?php _e('Attachment:', 'additional-tags'); ?></label>
				...
				<div class="trx_emailer_block">
					<fieldset class="trx_emailer_block_inner">
						<legend> <?php _e('Subscribers', 'additional-tags'); ?> </legend>
						<div class="trx_emailer_fields">
							<div class="trx_emailer_field trx_emailer_group">
								<label for="emailer_group"><?php _e('Select group:', 'additional-tags'); ?></label>
								<select name="emailer_group" id="emailer_group">
									<option value="none"<?php themerex_show_layout($group=='none' ? ' selected="selected"' : ''); ?>><?php _e('- Select group -', 'additional-tags'); ?></option>
									<?php
								...
								<input type="checkbox" name="emailer_subscribers_update" id="emailer_subscribers_update" value="1"<?php themerex_show_layout( $subscribers_update ? ' checked="checked"' : ''); ?> /><label for="emailer_subscribers_update" class="inline" title="<?php _e('Update the subscribers list for selected group', 'additional-tags'); ?>"><?php _e('Update', 'additional-tags'); ?></label>
								<input type="checkbox" name="emailer_subscribers_clear" id="emailer_subscribers_clear" value="1"<?php themerex_show_layout( $subscribers_clear ? ' checked="checked"' : ''); ?> /><label for="emailer_subscribers_clear" class="inline" title="<?php _e('Clear this group from not confirmed emails after send', 'additional-tags'); ?>"><?php _e('Clear', 'additional-tags'); ?></label>
								<input type="checkbox" name="emailer_subscribers_delete" id="emailer_subscribers_delete" value="1"<?php themerex_show_layout( $subscribers_delete ? ' checked="checked"' : ''); ?> /><label for="emailer_subscribers_delete" class="inline" title="<?php _e('Delete this group after send', 'additional-tags'); ?>"><?php _e('Delete', 'additional-tags'); ?></label>
							</div>
							<div class="trx_emailer_field trx_emailer_subscribers2">
								<label for="emailer_subscribers" class="big"><?php _e('List of recipients:', 'additional-tags'); ?></label>
							...
							<div class="trx_emailer_field trx_emailer_sender_name">
								<label for="emailer_sender_name"><?php _e('Sender name:', 'additional-tags'); ?></label>
							...
							<div class="trx_emailer_field trx_emailer_sender_email">
								<label for="emailer_sender_email"><?php _e('Sender email:', 'additional-tags'); ?></label>
				...
				<div class="trx_emailer_buttons">
					<a href="#" id="trx_emailer_send"><?php echo _e('Send', 'additional-tags'); ?></a>
				</div>
	...
	function themerex_callback_emailer_submit() {
		...
		if (preg_match('/[\.\-_A-Za-z0-9]+?@[\.\-A-Za-z0-9]+?[\ .A-Za-z0-9]{2,}/', $email)) {
			...
			if (isset($subscribers[$group][$email]))
				$response['error'] = __('E-mail address already in the subscribers list!', 'additional-tags');
			else {
				$subscribers[$group][$email] = md5(mt_rand());
				update_option('themerex_emailer_subscribers', $subscribers);
				$subj = sprintf(__('Site %s - Subscribe confirmation', 'additional-tags'), get_bloginfo('site_name'));
			...
			$response['error'] = __('E-mail address is not valid!', 'additional-tags');
		...
		if (preg_match('/[\.\-_A-Za-z0-9]+?@[\.\-A-Za-z0-9]+?[\ .A-Za-z0-9]{2,}/', $email)) {
			...
			if (isset($subscribers[$group][$email])) {
				if ($subscribers[$group][$email] == $nonce) {
					...
					update_option('themerex_emailer_subscribers', $subscribers);
					themerex_set_system_message(__('Confirmation complete! E-mail address succefully added in the subscribers list!', 'additional-tags'), 'success');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url() );
				} else if ($subscribers[$group][$email] != '') {
					themerex_set_system_message(__('Bad confirmation code!', 'additional-tags'), 'error');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url() );
				} else {
					themerex_set_system_message(__('E-mail address already exists in the subscribers list!', 'additional-tags'), 'error');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url() );
				}
			}
