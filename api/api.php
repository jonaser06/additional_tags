<?php
/**
 * ThemeREX Addons Third-party plugins API
 *
 * @package WordPress
 * @subpackage ThemeREX Addons
 * @since v1.6.29
 */

// Don't load directly
if ( ! defined( 'ABSPATH' ) ) {
	die( '-1' );
}

// Define list with api
if (!function_exists('trx_addons_api_load')) {
	add_action( 'after_setup_theme', 'trx_addons_api_load', 2 );
	add_action( 'trx_addons_action_save_options', 'trx_addons_api_load', 2 );
	function trx_addons_api_load() {
		static $loaded = false;
		if ($loaded) return;
		$loaded = true;
		global $TRX_ADDONS_STORAGE;
		$TRX_ADDONS_STORAGE['api_list'] = apply_filters('trx_addons_api_list', array(
			
			'bbpress' => array(
							'title' => __('BB Press & Buddy Press', 'additional-tags')
						),
			'booked' => array(
							'title' => __('Booked Appointments', 'additional-tags')
						),
			'calculated-fields-form' => array(
							'title' => __('Calculated Fields Form', 'additional-tags')
						),
			'contact-form-7' => array(
							'title' => __('Contact Form 7', 'additional-tags')
						),
			'content_timeline' => array(
							'title' => __('Content Timeline', 'additional-tags')
						),
			'easy-digital-downloads' => array(
							'title' => __('Easy Digital Downloads', 'additional-tags')
						),
			'essential-grid' => array(
							'title' => __('Essential Grid', 'additional-tags')
						),
			'instagram-feed' => array(
							'title' => __('Instagram Feed', 'additional-tags')
						),
			'mailchimp-for-wp' => array(
							'title' => __('MailChimp for WordPress', 'additional-tags')
						),
			'mp-timetable' => array(
							'title' => __('MP TimeTable', 'additional-tags')
						),
			'revslider' => array(
							'title' => __('Revolution Slider', 'additional-tags')
						),
			'siteorigin-panels' => array(
							'title' => __('SiteOrigin Panels (free PageBuilder)', 'additional-tags'),
							// Always enabled!!!
							'std' => 1,
							'hidden' => false
						),
			'the-events-calendar' => array(
							'title' => __('The Events Calendar', 'additional-tags'),
							'layouts_sc' => array(
								'default'	=> esc_html__('Default', 'additional-tags'),
								'detailed'	=> esc_html__('Detailed', 'additional-tags')
							)
						),
			'tourmaster' => array(
							'title' => __('Tour Master', 'additional-tags')
						),
			'trx_donations' => array(
							'title' => __('ThemeREX Donations', 'additional-tags')
						),


			'ubermenu' => array(
							'title' => __('UberMenu', 'additional-tags')
						),
			'js_composer' => array(
							'title' => __('WPBakery PageBuilder', 'additional-tags'),
							// Always enabled!!!
							'std' => 1,
							'hidden' => false
						),
			'vc-extensions-bundle' => array(
							'title' => __('VC Extensions Bundle', 'additional-tags')
						),
			'woocommerce' => array(
							'title' => __('WooCommerce', 'additional-tags')
						),
			'sitepress-multilingual-cms' => array(
							'title' => __('WPML - Sitepress Multilingual CMS', 'additional-tags')
						),
			)
		);
		if (is_array($TRX_ADDONS_STORAGE['api_list']) && count($TRX_ADDONS_STORAGE['api_list']) > 0) {
			foreach ($TRX_ADDONS_STORAGE['api_list'] as $w=>$params) {
				if (empty($params['preloaded'])
					&& ($fdir = (TRX_ADDONS_PLUGIN_DIR . "api/{$w}/{$w}.php")) != '') {
					include_once $fdir;
				}
			}
		}
	}
}

//-----------------------------------------------------------------------------------
//-- CHECK FOR COMPONENTS EXISTS
//--  Attention! This functions are used in many files and must be declared here!!!
//-----------------------------------------------------------------------------------

// Check if plugin 'WPBakery PageBuilder' is installed and activated
if ( !function_exists( 'trx_addons_exists_visual_composer' ) ) {
	function trx_addons_exists_visual_composer() {
		return class_exists('Vc_Manager');
	}
}

// Check if plugin 'SiteOrigin Panels' is installed and activated
if ( !function_exists( 'trx_addons_exists_sop' ) ) {
	function trx_addons_exists_sop() {
		return class_exists('SiteOrigin_Panels');
	}
}

// Check if any PageBuilder is installed and activated
if ( !function_exists( 'trx_addons_exists_page_builder' ) ) {
	function trx_addons_exists_page_builder() {
		return trx_addons_exists_visual_composer() || trx_addons_exists_sop();
	}
}

// Check if RevSlider installed and activated
if ( !function_exists( 'trx_addons_exists_revslider' ) ) {
	function trx_addons_exists_revslider() {
		return function_exists('rev_slider_shortcode');
	}
}

// Check if plugin installed and activated
if ( !function_exists( 'trx_addons_exists_woocommerce' ) ) {
	function trx_addons_exists_woocommerce() {
		return class_exists('Woocommerce');
	}
}

// Check if plugin installed and activated
if ( !function_exists( 'trx_addons_exists_edd' ) ) {
	function trx_addons_exists_edd() {
		return class_exists('Easy_Digital_Downloads');
	}
}

// Check if plugin installed and activated
if ( !function_exists( 'trx_addons_exists_wpml' ) ) {
	function trx_addons_exists_wpml() {
		return defined('ICL_SITEPRESS_VERSION') && class_exists('sitepress');
	}
}
?>