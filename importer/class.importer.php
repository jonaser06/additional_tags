...
class trx_addons_demo_data_importer {
	...
	function admin_menu_item() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			add_theme_page(esc_html__('Install Demo Data', 'additional-tags'), esc_html__('Install Demo Data', 'additional-tags'), 'edit_theme_options', 'trx_importer', array($this, 'build_page'));
		}
	}
	...
	function admin_scripts() {
		...
		wp_localize_script('trx_addons-importer', 'TRX_ADDONS_GLOBALS', array( 'admin_mode'	=> true,
            'ajax_nonce' => wp_create_nonce('ajax_nonce'),
            'ajax_url'	=> admin_url('admin-ajax.php'),
            'user_logged_in' => true,
            'msg_importer_full_alert' => esc_html__("ATTENTION!\n\nIn this case ALL THE OLD DATA WILL BE ERASED\nand YOU WILL GET A NEW SET OF POSTS, pages and menu items.", 'additional-tags')
                    . "\n\n"
                    . esc_html__("It is strongly recommended only for new installations of WordPress\n(without posts, pages and any other data)!", 'additional-tags')
                    . "\n\n"
                    . esc_html__("Press 'OK' to continue or 'Cancel' to return to a partial installation", 'additional-tags')
            )
        );
	}
	...
	function build_page() {
		...
		if ( isset($_POST['exporter_action']) ) {
			if ( !wp_verify_nonce( trx_addons_get_value_gp('nonce'), admin_url() ) )
				$this->error = esc_html__('Incorrect WP-nonce data! Operation canceled!', 'additional-tags');
			else
				$this->exporter();
		}
		...
		?><div class="trx_importer"><?php
			...
			?><div class="trx_importer_section">
				...
				<h2 class="trx_importer_title"><?php esc_html_e('Importer', 'additional-tags'); ?></h2>
				...
				<form id="trx_importer_form">
					...
					<?php if (count($this->options['files']) > 1) { ?>
						<p><b><?php esc_html_e('Select the demo to be imported:', 'additional-tags'); ?></b></p>
					...
					<p><b><?php esc_html_e('Select the demo-data set to be imported:', 'additional-tags'); ?></b></p>
					<div class="trx_importer_demo_set">
						<label><input type="radio"<?php if ($this->options['demo_set']=='part') echo ' checked="checked"'; ?> value="part" name="demo_set" /><?php esc_html_e('Only pages, forms and sliders', 'additional-tags'); ?></label>
						<div class="trx_importer_description trx_importer_description_part<?php if ($this->options['demo_set']!='part') echo ' trx_importer_hidden'; ?>">
							<ol>
								<li><?php echo wp_kses_data(__('<b>Only pages, forms layouts and sliders will be imported in this case.</b>', 'additional-tags')); ?></li>
								<li><?php echo wp_kses_data(__('The imported data will be added to the existing content.', 'additional-tags')); ?></li>
								<li><?php echo wp_kses_data(__('All the images are replaced with placeholders.', 'additional-tags')); ?></li>
								<li><?php echo wp_kses_data(__('The new pages provide sample markup and shall not be included in the menu! You have to do it yourself.', 'additional-tags')); ?></li>
								<li><?php echo wp_kses_data(__('Import some components (Revolution sliders, Essential Grids, etc.) take a long time - <b>please wait until the end of the procedure, do not navigate away from this page</b>.', 'additional-tags')); ?></li>
							</ol>
						</div>
						<label><input type="radio"<?php if ($this->options['demo_set']=='full') echo ' checked="checked"'; ?> value="full" name="demo_set" /><?php esc_html_e('Whole demo-site content', 'additional-tags'); ?></label>
						<div class="trx_importer_description trx_importer_description_full<?php if ($this->options['demo_set']!='full') echo ' trx_importer_hidden'; ?>">
							<ol>
								<li><?php echo wp_kses_data(__('<b>ALL EXISTING CONTENTS</b> of your website will be deleted and replaced with the new data!', 'additional-tags')); ?></li>
								<li><?php echo wp_kses_data(__('<b>It is recommended for new installations of WordPress</b> (without posts, pages and any other data)!', 'additional-tags')); ?></li>
								<li><?php echo wp_kses_data(__('As a result, you will get a new set of posts, pages, menu items - <b>a complete copy of our demo site</b>.', 'additional-tags')); ?></li>
								<li><?php echo wp_kses_data(__('Import some components (Revolution sliders, Essential Grids, etc.) take a long time - <b>please wait until the end of the procedure, do not navigate away from this page</b>.', 'additional-tags')); ?></li>
							</ol>
						</div>
					</div>
                    ...
                    <div class="trx_importer_advanced_settings_wrap<?php if ($this->options['demo_set']=='part') echo ' trx_importer_advanced_settings_opened'; ?>">
                        <p class="trx_importer_subtitle trx_importer_advanced_settings_title"><?php esc_html_e('Advanced settings', 'additional-tags'); ?></p>
                        <div class="trx_importer_advanced_settings">
                            <p class="trx_importer_subtitle"><?php esc_html_e('Select the elements to be imported:', 'additional-tags'); ?></p>

					<?php
					$this->show_importer_params(array(
						'slug' => 'posts',
						'title' => esc_html__('Import posts, pages, taxonomies, etc.', 'additional-tags'),
						'part' => 1,
						'checked' => true,
					...
					$this->show_importer_params(array(
						'slug' => 'tm',
						'title' => esc_html__('Import Theme Options', 'additional-tags'),
						'part' => 1,
						'checked' => true
					));
					$this->show_importer_params(array(
						'slug' => 'to',
						'title' => esc_html__('Import Theme Settings', 'additional-tags'),
						'part' => 1,
						'checked' => true
					));
					$this->show_importer_params(array(
						'slug' => 'widgets',
						'title' => esc_html__('Import Widgets', 'additional-tags'),
						'part' => 1,
						'checked' => true,
						'class' => 'trx_importer_separator'
					));
					...
					$this->show_importer_params(array(
						'slug' => 'uploads',
						'title' => esc_html__('Import media', 'additional-tags'),
						'part' => 1,
						'checked' => true,
					...
					if (!empty($this->options['regenerate_thumbnails'])) {
						$this->show_importer_params(array(
							'slug' => 'thumbnails',
							'title' => esc_html__('Regenerate thumbnails', 'additional-tags'),
							'part' => 1,
							'checked' => true
						));
					}
					...
					<div class="trx_buttons">
						<input type="button" value="<?php esc_attr_e('Start import', 'additional-tags'); ?>">
					</div>
				</form>
				
				<div id="trx_importer_progress" class="notice notice-info style_<?php echo esc_attr($this->options['demo_style']); ?>">
					<h4 class="trx_importer_progress_title"><?php esc_html_e('Import demo data', 'additional-tags'); ?></h4>
					<table border="0" cellpadding="4">
					<?php

					// Show first part of import fields
					$fields = array(
						'posts'		=> esc_html__('Posts', 'additional-tags'),
						'tm'		=> esc_html__('Theme Options', 'additional-tags'),
						'to'		=> esc_html__('Theme Settings', 'additional-tags'),
						'widgets'	=> esc_html__('Widgets', 'additional-tags'),
					);
					...
					$fields = array(
						'uploads'	=> esc_html__('Media', 'additional-tags'),
						'thumbnails'=> esc_html__('Thumbnails', 'additional-tags')
					);
					foreach ($fields as $slug=>$title) {
						$this->show_importer_fields(array('slug' => $slug, 'title' => $title));
					}
					...
					<h4 class="trx_importer_progress_complete"><?php esc_html_e('Congratulations! Data import complete!', 'additional-tags'); ?> <a href="<?php echo esc_url(home_url('/')); ?>"><?php esc_html_e('View site', 'additional-tags'); ?></a></h4>
				</div>
			...
			?><div class="trx_exporter_section">
				...
				<h2 class="trx_title"><?php esc_html_e('Exporter', 'additional-tags'); ?></h2>
				...
				<form id="trx_exporter_form" action="#" method="post">
					...
					if ( isset($_POST['exporter_action']) ) { 
						...
						$fields = array(
							'users'			=> esc_html__('Users', 'additional-tags'),
							'posts'			=> esc_html__('Posts', 'additional-tags'),
							'uploads'		=> esc_html__('Uploads', 'additional-tags'),
							'theme_mods'	=> esc_html__('Theme Options', 'additional-tags'),
							'theme_options'	=> esc_html__('Theme Settings', 'additional-tags'),
							'widgets'		=> esc_html__('Widgets', 'additional-tags'),
						);
						foreach ($fields as $slug=>$title) {
						...
						if (false && count($this->options['files']) > 1) {
							...
							?><p><b><?php esc_html_e('Select the demo type to be exported', 'additional-tags'); ?></b></p><?php
						...
						?>
						<div class="trx_buttons">
							<input type="submit" value="<?php esc_attr_e('Export Demo Data', 'additional-tags'); ?>">
						</div>
						<?php
					}
					?>
	...
	function show_exporter_fields($args=array()) {
		...
		<tr>
			...
			<td><a download="<?php echo esc_attr(!empty($args['download']) ? $args['download'] : $args['slug'].'.txt'); ?>" href="<?php echo esc_url(TRX_ADDONS_PLUGIN_URL . 'importer/export/'.trim($args['slug']).'.txt'); ?>"><?php esc_html_e('Download', 'additional-tags'); ?></a></td>
		</tr>
		<?php
	}
	...
	function check_required_plugins($list='') {
		...
		if ($not_installed) {
			$this->error = '<b>'.esc_html__('Attention! For correct installation of the selected demo data, you must install and activate the following plugins: ', 'additional-tags').'</b><br>'.($not_installed);
			return false;
		}
		return true;
	}
	...
	function exporter() {
		...
		if (is_array($rows) && count($rows)>0) {
			foreach ($rows as $k=>$v) {
				...
				$rows[$k]['user_pass']		= '';
				$rows[$k]['display_name']	= sprintf(esc_html__('User %d', 'additional-tags'), $v['ID']);
		...
		if (is_array($rows) && count($rows)>0) {
			foreach ($rows as $k=>$v) {
				...
				else if ($v['meta_key'] == 'first_name')			$rows[$k]['meta_value'] = sprintf(esc_html__('FName%d', 'additional-tags'), $v['user_id']);
				else if ($v['meta_key'] == 'last_name')				$rows[$k]['meta_value'] = sprintf(esc_html__('LName%d', 'additional-tags'), $v['user_id']);
				else if ($v['meta_key'] == 'billing_first_name')	$rows[$k]['meta_value'] = sprintf(esc_html__('FName%d', 'additional-tags'), $v['user_id']);
				else if ($v['meta_key'] == 'billing_last_name')		$rows[$k]['meta_value'] = sprintf(esc_html__('LName%d', 'additional-tags'), $v['user_id']);
	...
	function export_dump($table) {
		...
		if ( count( $wpdb->get_results( $wpdb->prepare( "SHOW TABLES LIKE %s", $wpdb->prefix . trim($table) ), ARRAY_A ) ) == 1 ) {
			...
			if ($this->options['debug']) dfl(sprintf(__("Export %d rows from table '%s'. Used query: %s", 'additional-tags'), count($rows), $table, $query));
		}
		return $rows;
	}
	...
	function importer() {
		if ($this->options['debug']) dfl(__('AJAX handler for importer', 'additional-tags'));
		...
		if ($this->options['debug']) dfl( sprintf(__('Dispatch action: %s', 'additional-tags'), $this->action) );
		...
		if ($this->action == 'import_start') {
			...
			if ($this->options['debug']) dfl(sprintf(__('Start import from "%s"', 'additional-tags'), $this->options['demo_url']));
		...
		if ($this->options['debug']) dfl( sprintf(__("AJAX handler finished - send results to client: %s", 'additional-tags'), json_encode($this->response)) );
		...
		if ($this->options['demo_set']=='full') {
			if (strpos($this->options['demo_parts'], 'posts')!==false) {
				if ($this->options['debug']) 
					dfl( __('Clear tables', 'additional-tags') );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->posts}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table POSTS.', 'additional-tags') . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->postmeta}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table POSTMETA.', 'additional-tags') . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->comments}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table COMMENTS.', 'additional-tags') . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->commentmeta}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table COMMENTMETA.', 'additional-tags') . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->terms}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table TERMS.', 'additional-tags') . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->term_relationships}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table TERM_RELATIONSHIPS.', 'additional-tags') . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->term_taxonomy}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table TERM_TAXONOMY.', 'additional-tags') . ' ' . ($res->get_error_message()) );
			}
	...
	function import_users() {
		if ($this->options['debug']) 
			dfl(__('Import users', 'additional-tags'));
		$this->response['start_from_id'] = 0;
		$this->import_dump('users', __('Users', 'additional-tags'));
	}

	// Import posts, terms and comments
	function import_posts() {
		if ($this->options['debug']) 
			dfl(__('Import posts, terms and comments', 'additional-tags'));
		...
		$this->import_dump('posts', __('Posts', 'additional-tags'));
	}

	// Import media (uploads folder)
	function import_uploads() {
		if ($this->options['debug']) 
			dfl(__('Import media', 'additional-tags'));
		...
		foreach ($files as $file) {
			...
			// Append next part (*.001, *.002 ...) to archive
				...
				if ( ($txt = $this->get_file($file, round(max(0, $counter-1) / count($files) * 100))) === false)
					...
					if ($this->options['debug']) 
						dfl(sprintf( __('Loaded %d bytes', 'additional-tags'), $res));
			...
			// Check time
			if ($this->options['debug']) 
				dfl(sprintf( __('File %s imported. Current import progress: %s. Time limit: %s sec. Elapsed time: %s sec.', 'additional-tags'), $file, $result.'%', $this->max_time, time() - $this->start_time));
			// Unzip file
			if ($need_extract) {
				if (!empty($zip) && file_exists($zip)) {
					if ($this->options['debug']) 
						dfl(sprintf(__('Extract zip-file "%s"', 'additional-tags'), $zip));
					$rez = unzip_file($zip, $this->uploads_dir);
					if ( is_wp_error($rez) ) {
						$msg = sprintf(__('Error when unzip file "%s"', 'additional-tags'), $zip);
					...
					$msg = sprintf(__('File "%s" not found', 'additional-tags'), $zip);
					$this->response['error'] = $msg;
					if ($this->options['debug']) 
						dfl($msg);
				}
	...
	function import_thumbnails() {
		if ($this->options['debug']) 
			dfl(__('Regenerate thumbnails', 'additional-tags'));
		...
		foreach ($files as $id=>$file) {
			...
			// Check time
			if ($this->options['debug']) 
				dfl(sprintf( __('Thumbnails of the attachments %s: %s regenerated. Current import progress: %s. Time limit: %s sec. Elapsed time: %s sec.', 'additional-tags'), $id, $file, $result.'%', $this->max_time, time() - $this->start_time));
			// Break import after timeout or if attachments loading from parts - to show percent loading after each part
	...
	function import_theme_mods() {
		if ($this->options['debug']) 
			dfl(__('Import Theme Options (WP modifications)', 'additional-tags'));
		...
		if (is_array($data) && count($data) > 0) {
			...
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'additional-tags'), $this->options['files'][$this->options['demo_type']]['file_with_mods']));
		}
	}
	...
	function import_theme_options() {
		if ($this->options['debug']) 
			dfl(__('Import Theme Settings', 'additional-tags'));
		...
		if (is_array($data) && count($data) > 0) {
			...
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'additional-tags'), $this->options['files'][$this->options['demo_type']]['file_with_options']));
		}
	}
	...
	function import_widgets() {
		if ($this->options['debug']) 
			dfl(__('Import Widgets', 'additional-tags'));
		...
		if (is_array($data) && count($data) > 0) {
			...
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'additional-tags'), $this->options['files'][$this->options['demo_type']]['file_with_widgets']));
		}
	}
	...
	function import_dump($slug, $title) {
		if ($this->options['debug'])
			dfl(sprintf(__('Import dump file: "%s"', 'additional-tags'), $this->options['files'][$this->options['demo_type']]['file_with_' . $slug]));
		...
		if (is_array($data) && count($data) > 0) {
			...
			foreach ($data as $table=>$rows) {
				...
				$break = false;
				if ($this->options['debug'])
					dfl(sprintf(__('Process table "%s"', 'additional-tags'), $table));
				if ( count( $wpdb->get_results( $wpdb->prepare( "SHOW TABLES LIKE %s", $wpdb->prefix . trim($table) ), ARRAY_A ) ) == 0 ) {
					if ($this->options['debug'])
						dfl(sprintf(__('Table "%s" does not exists! Skip dump import for this table.', 'additional-tags'), $table));
					continue;
				}
				...
				if (is_array($rows) && ($posts_all=count($rows)) > 0) {
					...
					foreach ($rows as $row) {
						...
						if ($this->options['debug']) {
							dfl( sprintf( __('Record (ID=%s) is imported. Progress: %s. Time: %s sec. from %s sec.', 'additional-tags'),
											!empty($row['ID']) 
				...
				if ($this->options['debug']) dfl(sprintf(__('Imported %s. Elapsed time %s sec. of %s sec.', 'additional-tags'), $result.'%', time() - $this->start_time, $this->max_time));
				if ($break) break;
			}
		} else {
			if ($this->options['debug']) 
				dfl(sprintf(__('Error unserialize data from the file %s', 'additional-tags'), $this->options['files'][$this->options['demo_type']]['file_with_' . $slug]));
		}
		$this->response['result'] = $result;
	}
	...
	function import_prepare_no_image() {
		$no_image_title = esc_html__('No-Image placeholder', 'additional-tags');
	...
	function get_list_pages_callback() {
		...
		$response = array(
			'error' => empty($_POST['demo_type']) ? esc_html__('Incorrect parameters', 'additional-tags') : '',
		);
	...
	function download_file($fname, $result=0) {
		...
		if (substr($fname, 0, 5)=='http:' || substr($fname, 0, 6)=='https:') {
			...
			if (is_string($response)) {
				...
				if ($this->options['debug']) 
					dfl(sprintf(__('Download file %s successful', 'additional-tags'), $fname));
			} else {
				if ($attempt < 3) {
					$this->response['attempt'] = $attempt;
					$this->response['result'] = $result;
					if ($this->options['debug']) {
						$error_log = sprintf(__("Attempt %d. Download file '%s' failed.", 'additional-tags'), $attempt, $fname);
						dfl($error_log);
					}
				} else {
					unset($this->response['attempt']);
					$this->response['error'] = sprintf(__("Error download file '%s'.", 'additional-tags'), $fname)
												. " \n" . __("Please, try again!", 'additional-tags');
					if ($this->options['debug']) 
	...
	function get_file($fname, $result=0) {
		...
		if (empty($txt)) {
			if ($attempt < 3) {
				...
				$this->response['result'] = $result;
				if ($this->options['debug']) {
					$error_log = sprintf(__("Attempt %d. Load data from the file '%s' failed. ", 'additional-tags'), $attempt, $fname);
					dfl($error_log);
				}
			} else {
				unset($this->response['attempt']);
				$this->response['error'] = sprintf(__("Error load data from the file '%s'.", 'additional-tags'), $fname)
											. " \n" . __("Please, try again!", 'additional-tags');
				if ($this->options['debug']) 
					dfl($this->response['error']);
			}
			...
			unset($this->response['attempt']);
			if ($this->options['debug']) 
				dfl(sprintf(__('Load data from the file %s successful', 'additional-tags'), $fname));
		}
		return $txt;
	}
}
